    ...
    try {
        ...
        client->virtual_hid_keyboard_ready.connect([copy](auto&& ready) {
            ...
            if (ready && !was_ready) {
                ...
                // socket (e.g. a key Release) leaves the device holding that
                // key down until the daemon garbage-collects the dead client
                // (1-3s of OS-level autorepeat — KeyPath MAL-57). Reset clears
                // device state as soon as the keyboard is usable again; the
