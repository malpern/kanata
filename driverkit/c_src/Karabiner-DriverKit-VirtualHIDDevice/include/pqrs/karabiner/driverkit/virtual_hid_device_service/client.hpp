...
private:
  std::string client_socket_file_path(void) const {
    // KeyPath (MAL-57): use a stable per-process path instead of a fresh
    // timestamped one per connection. The daemon keys its per-client state
    // (including the DriverKit virtual keyboard instance) by this path, so a
    // reconnect under the same path reuses the existing keyboard instead of
    // spawning a second instance while the old one — possibly holding a key
    // whose Release was lost in the dying socket — autorepeats until the
    // daemon garbage-collects it seconds later. With a stable path the
    // post-reconnect virtual_hid_keyboard_reset reaches the same instance
    // and clears any stuck key immediately. local_datagram::client_impl
    // removes an existing file before bind, so stale files are harmless.
    std::stringstream ss;
  ...
  void create_client(void) {
    ...
    // KeyPath (MAL-57): 1000ms instead of upstream's 3000ms. Detecting a dead
    // daemon connection faster shrinks the window in which a key whose
    // Release was lost in transit keeps autorepeating at the OS level. The
    // daemon already pings each client at 1000ms, so this adds no new traffic
    // class.
    client_->set_server_check_interval(std::chrono::milliseconds(1000));
