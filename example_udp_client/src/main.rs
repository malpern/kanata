...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        let message = match input {
            ...
            _ => {
                println!("Unknown command. Available: layers, current, info, change <name>, reload, quit");
                continue;
            }
        };
            ...
            ClientMessage::Reload { .. } => {
                ...
                match socket.recv_from(&mut buf) {
                    Ok((size, _)) => {
                        if let Ok(response) = serde_json::from_slice::<ServerMessage>(&buf[..size]) {
                            match response {
                                ...
                                ServerMessage::CurrentLayerInfo { name, cfg_text } => {
                                    println!("Current layer: {}", name);
                                    println!("Config snippet: {}", cfg_text.chars().take(100).collect::<String>());
                                }
                                ServerMessage::Error { msg } => {
                                    println!("❌ Error: {}", msg);
                                }
                        ...
                        } else if let Ok(response) = serde_json::from_slice::<ServerResponse>(&buf[..size]) {
                            match response {
                                ServerResponse::Ok => {
